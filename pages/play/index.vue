<template>
	<div class="play-container">
      ...
      ></div> -->
		...
		<view class="author" :style="{ top: CustomBar + 'px' }">{{ playInfo.singer }}</view>
		<view class="img-container" :style="{ transform: 'translate(-50%, -50%) rotate(' + rotate + 'deg)' }" v-show="!lyricShow">
			<image :src="playInfo.coverImgUrl" class="authorImg" @click="getLyric"></image>
		</view>
		<scroll-view
			scroll-y
			v-if="lyricShow"
		...
		<view class="bottom-control">
			...
			<view class="iconList flex">
				<text
					class="iconfont"
					:class="
            loginType === 1 ? likeList.includes(playInfo.id)
              ? 'icon-like lighIcon'
              : 'icon-unlike' : likeIndex != -1 ? 'icon-like lighIcon'
              : 'icon-unlike'
          "
					@click="likeMusic()"
				...
				<text class="iconfont" :class="!paused ? 'icon-play' : 'icon-pause'" style="font-size: 90rpx; margin: 0 35px" @click="playMusic"></text>
		...
		methods: {
			initLyric() {
				...
				this.currentLyricIndex = this.lyricList.findIndex((item, index) => {
					return item.time < timeStamp && this.lyricList[index + 1] ? this.lyricList[index + 1].time > timeStamp : true;
				});
			...
			async likeMusic() {
				if (this.loginType === 1) {
					if (this.playInfo.type === 'netease') {
						...
						uni.$u.toast('扫码登录无法收藏此音乐!');
					}
				} else {
					const searchObj = { name: this.playInfo.title, singer: this.playInfo.singer, platform: this.playInfo.type };
					const index = _.findIndex(this.likeList, searchObj);
					const params = {
						userid: this.serverUserInfo.userId,
						quality: this.playInfo.quality,
						pic: this.playInfo.coverImgUrl,
						...searchObj,
					};
					if (index != -1) {
						await this.$api.deleteCollect(params);
					} else {
						await this.$api.addCollect(params);
					}
					await this.getLikeData();
					this.likeIndex = _.findIndex(this.likeList, searchObj);
				}
			},
			...
			async getLyric(bool) {
				...
				if (this.playInfo.type === 'netease') {
					...
					let res = {};
					res = await this.$api.getLyrics({ type: 'kugou', keyword: this.playInfo.title });
					if (res.data.search('没有歌词') != -1) {
						res = await this.$api.getLyrics({ type: '163', keyword: this.playInfo.title });
					}
					let tempData = res.data.split('\n');
					let index = _.findIndex(tempData, (item) => item.search('00:00') != -1);
					data = _.slice(tempData, index, tempData.length);
				}
				let timeReg = /^\[.*\]/;
				let json = [];
				data.forEach((item) => {
					if (item.match(timeReg)) {
						...
						json.push({
							...
							content: this.playInfo.type === 'netease' ? item.substr(11) : item.substr(10),
						});
					}
				});
				this.lyricList = json;
